...
@Composable
fun <T> TypedSelectField(
    modifier: Modifier = Modifier,
    label: String = "Select",
    initial: T?,
    options: List<T>,
    labelGetter: KProperty1<T, String>,
    onSelectionChange: (T) -> Unit
) {
    var dropDownState by remember { mutableStateOf(DropdownMenuState(DropdownMenuState.Status.Closed)) }
    var selectedOption by remember { mutableStateOf<T?>(initial ?: options.firstOrNull()) }
    ...
    Row(
        ...
        OutlinedTextField(
            ...
            value = selectedOption?.let(labelGetter) ?: "",
            trailingIcon = {
                Icon(
